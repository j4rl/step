...
<script>
    // URL till JSON-data
const url = '_json_teamdata.php?comp=4';
...
function presentTeamData(teamData, vdo) {
  console.log(`${vdo} ${teamData[0].team}:`);

  const totalSteps = teamData.reduce((acc, member) => acc + member.user_steps, 0);
  console.log(`Kvot för laget: ${teamData[0].team_quota}`);
  console.log(`Totala steg för laget: ${totalSteps}`);
  teamData.forEach(member => {
    console.log(`- ${member.user}: Steg: ${member.user_steps}`);
  });
  console.log();
}
